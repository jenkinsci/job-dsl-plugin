...
class FreeStyleJobSpec extends Specification {
    ...
    private FreeStyleJob job = new FreeStyleJob(jobManagement)
    ...
    def 'load an empty template from a manually constructed job and generate xml from it'() {
        ...
        then:
        _ * jobManagement.getConfig('TMPL') >> FreeStyleJob.TEMPLATE
    ...
    def 'update Node using withXml'() {
        setup:
        Node project = new XmlParser().parse(new StringReader(FreeStyleJob.TEMPLATE))
        FreeStyleJob job = new FreeStyleJob(null)
